...
mtop_fig5_caption <- paste("Cumulative midge effect on total bottom-up control",
                           "(\"BU total\") for detritivores,",
                           "net top-down control (\"TD net\"),",
...
```{r M-to-one-pool-fig5, fig.width=6.5, fig.height=7.5, fig.cap=mtop_fig5_caption}
...
mtop_fig5_plot <- function(.midges_not_to, 
    ...
    # .midges_not_to = "D"; .ylims = c(-0.688, 4.305); .pool = "detritivore"
    ...
    mt_plot <- mt_combo %>%
        ...
        ggplot(aes(area, value)) +
        geom_hline(yintercept = 0, color = "gray80") +
        geom_line(aes(color = direction, linetype = type, group = id), size = 0.75) +
        ggtitle(.title) +
        facet_grid(hM ~ q) +
        ...
        theme(legend.background = element_blank(),
              ...
              strip.background = element_blank(),
              strip.text.y = element_text(vjust = 0.5, hjust = 0, size = 11, angle = 0),
