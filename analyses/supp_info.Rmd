...
vary_iI_mN <- function(.iI, .mN) {
    pools <- equil_pools(iI = .iI, mN = .mN)
    sim <- food_web(tmax = 100, s = 10, b = 50, w = 20,
                    other_pars = list(f = 3, iI = .iI, mN = .mN), 
                    ep_obj = pools) %>%
        filter(pool != "midge") %>%
        mutate(pool = droplevels(pool)) %>% 
        group_by(pool) %>%
