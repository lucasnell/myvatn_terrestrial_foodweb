...
web <- R6Class(
    ...
    active = list(
        ...
        initial_states = function(){
            data.frame(pool = c("nitrogen", "detritus", "plant", "detrivore",
                                "herbivore", "predator", "midge"),
                       N =  c(self$N0,self$D0,self$P0,self$V0,self$H0,self$R0,self$M0))
